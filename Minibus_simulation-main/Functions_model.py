...
def model_construction(params, N, P_nodes, P_sched, P_wait, D_nodes, P_and_D, S_nodes, Z_nodes, Q, 
    ...
    for k in bus_idx:
        model_MILP_base.addConstr(
            ...
            a_k[S_nodes[k], k] == e_dict[S_nodes[k]], ## NEWNEW
...
def run_simulation(params, stations, origin_stations, initial_K, bus_idx, bus_cost, get_travel_time_func):
    ...
    for t in pbar:
        ...
        for k in range(len(K)): # NEWNEW
            i = S_nodes[k] # NEWNEW
            e_dict[i] = max(t, K[k][2]) # e.g., max(10, 20) = 20 # NEWNEW
            l_dict[i] = 1440 # NEWNEW
        ...
        if model_MILP_base.status == gp.GRB.TIME_LIMIT and model_MILP_base.SolCount == 0: 
            ...
            # 2. Reset time limit to 8 minutes
            model_MILP_base.Params.TimeLimit = 480
        ...
        #if model_MILP_base.status in [gp.GRB.OPTIMAL, gp.GRB.TIME_LIMIT, gp.GRB.SOLUTION] and model_MILP_base.SolCount > 0:
        if model_MILP_base.SolCount > 0: ## NEWNEW
            ...
            for k in bus_idx:
                ...
                while curr_node != Z_nodes[k]:
                    ...
                    action_time = actual_arrival + w_sol.get((curr_node, k), 0) ## NEWNEW
                ...
                if is_active:
                    ...
                    if d_time > t + params["interval"]:
                        ...
                        for k in bus_idx:
                            if sum(x_sol[i, j, k] for j in N) > 0.5:
                                ...
                                p_time = a_sol[i, k] + w_sol.get((i, k), 0) ## NEWNEW
                                assigned_k = k
                                break
                    ...
                    ### 6.5.8 If drop-off happens within the current time step, we consider the request completed and do not carry it over
                        ...
                        for k in bus_idx:
                            ...
                            if sum(x_sol[i, j, k] for j in N) > 0.5:
                                if passenger_history[req_id]["time_picked_up"] is None:
                                    passenger_history[req_id]["time_picked_up"] = a_sol[i, k] + w_sol.get((i, k), 0) #max(a_sol[i, k], req_t_p[idx]) + w_sol.get((i, k), 0) ## NEWNEW
            ...
            for k in bus_idx:
                ...
                jump_time = max(t_next, max_a) ## NEWNEW

                if max_a == -1 and K[k][2] > t_next: ## NEWNEW
                    jump_time = K[k][2] ## NEWNEW

                for j in N: ## NEWNEW
                    if x_sol[last_visited_node, j, k] > 0.5 and a_sol[j, k] > t_next: ## NEWNEW
                        last_visited_node = j ## NEWNEW
                        jump_time = a_sol[j, k] ## NEWNEW
                        break ## NEWNEW

                K[k][1] = node_to_loc[last_visited_node]
                K[k][2] = jump_time ## NEWNEW
